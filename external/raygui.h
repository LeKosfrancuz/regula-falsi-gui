...
void GuiLoadStyle(const char *fileName)
{
    ...
    if (rgsFile != NULL)
    {
        ...
        {
         char* ret = fgets(buffer, MAX_LINE_BUFFER_SIZE, rgsFile);
         (void)ret;
        }
    ...
    if (tryBinary)
    {
        ...
        if (rgsFile != NULL)
        {
            ...
            if (fileDataSize > 0)
            {
                ...
                {
                 size_t ret = fread(fileData, sizeof(unsigned char), fileDataSize, rgsFile);
                 (void)ret;
                }
...
char **GuiLoadIcons(const char *fileName, bool loadIconsName)
{
    ...
    if (rgiFile != NULL)
    {
        ...
        {
         size_t ret;
         ret = fread(signature, 1, 4, rgiFile);
         ret = fread(&version, sizeof(short), 1, rgiFile);
         ret = fread(&reserved, sizeof(short), 1, rgiFile);
         ret = fread(&iconCount, sizeof(short), 1, rgiFile);
         ret = fread(&iconSize, sizeof(short), 1, rgiFile);
         (void)ret;
        }
            ...
            (signature[3] == ' '))
        {
            if (loadIconsName)
            {
                ...
                for (int i = 0; i < iconCount; i++)
                {
                    ...
                    {
                     size_t ret = fread(guiIconsName[i], 1, RAYGUI_ICON_MAX_NAME_LENGTH, rgiFile);
                     (void)ret;
                    }
                }
            }
            ...
            {
             size_t ret = fread(guiIconsPtr, sizeof(unsigned int), iconCount*(iconSize*iconSize/32), rgiFile);
             (void)ret;
            }
        }
